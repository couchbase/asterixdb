...
public class LSMColumnSampleLivenessSearchCursor extends LSMColumnBatchPointSearchCursor {

    public LSMColumnSampleLivenessSearchCursor(ILSMIndexOperationContext opCtx) {
        super(opCtx);
    }
    ...
    protected BTreeAccessor createAccessor(LSMComponentType type, BTree btree, int index) throws HyracksDataException {
        if (type == LSMComponentType.MEMORY) {
            // Row-format component: nothing column-specific to reduce. Sampling never reaches this branch (the
            // sample cursor's search components are all on-disk), but delegating keeps the class total.
        ...
        if (projectionInfo != null) {
            projectionInfo = projectionInfo.createExistenceOnlyProjectionInfo();
        }
    ...
    protected ITreeIndexCursor createCursor(LSMComponentType type, BTreeAccessor btreeAccessor) {
        if (btreeAccessor instanceof ColumnBTree.ColumnBTreeAccessor) {
            return ((ColumnBTree.ColumnBTreeAccessor) btreeAccessor).createExistencePointCursor();
        }
        // Memory (row-format) component; see createAccessor above.
        return super.createCursor(type, btreeAccessor);
    }
}
